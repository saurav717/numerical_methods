subroutine gaussJacobi(aIn, n, xGuess, tolerance, x)
  implicit none
  ...
  if(diagonallyDominant(coefficientMatrix,n,n) .eqv. .false.) then
    stop "error, the matrix is not diagonally dominant"
  end if
  ...
  do while(solutionFound .eqv. .false.)
    ...
    do rowCount = 1,n
      ...
      x(rowCount) = (1 / a(rowCount,columnCount)) * (a(rowCount,(n+1)) - factor)
    end do
